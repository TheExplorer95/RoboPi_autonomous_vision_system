...
class realCam:
    ...
    def __init__(self, depth=True, color=True, pixel=(640, 480), frames=30):
        ...
        if depth:
            self._config.enable_stream(rs.stream.depth, pixel[0],
                    pixel[1],rs.format.z16, framerate=frames)
        if color:
            self._config.enable_stream(rs.stream.color, pixel[0], pixel[1],
                    rs.format.bgr8, framerate=frames)
    ...
    def capture():
        ...
        self.depthColormap = cv.applyColorMap(cv.convertScaleAbs(imageDepth,
            alpha=0.03), cv.COLORMAP_JET)
        
        return None

    def show_video(name='Realsense'):
